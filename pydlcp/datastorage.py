"""
This class defines that storage for pydlcp data including acquisition parameters, data logging, and experiment results.
...
class H5Store:
    ...
    _create_h5(self):
        ...
        +-- /
             +-- logs: A group to store all logs
                   +-- temperature_and_current
             +-- bts: A group to store the bias-temperature stress CV results
                  +-- time
                  +-- voltage
                  +-- capacitance
                  +-- resistance
             +-- dlcp: A group to store all dlcp CV results
    ...
    def log_temperature_current(self, time: float, temperature: float, current: float):
    ...
    def get_bts_cv_at_time(self, time_idx: int) -> vcr_type:
    ...
    def append_cv(self, time: float, cv_data: vcr_type):
    ...
    def append_dlcp(self, time: float, dlcp_data: dlcp_type):
